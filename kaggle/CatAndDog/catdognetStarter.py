...
def catdog():
    ...
    model.add(Convolution2D(16, 3, 3, border_mode='same', input_shape=(3, ROWS, COLS), activation='relu'))
...
df = pd.Series([ row[0] for row in list(predictions)], index=[int(path.split('/')[-1].split('.')[0]) for path in test_images])

sortedDf= df.sort_index()
sortedDf.to_csv('submission.csv')